...
class Game:
    ...
    def timerData(self):
        if self.timer is not None and self.timer_start != 0:
            ...
            endGame = False if remain <= 0 else False #I changed the logic to run forever
            return (remain, endGame)
        return (None, False)

    def loop_events(self, input):
        hit = False
        miss = False
        clicked = False
        pos = (0, 0)
    ...
    def loop_display(self, clicked, hit, miss):
        ...
        if not endGame:
            ...
            if self.show_hit > 0 and time.get_ticks() - self.show_hit <= Constants.MOLEHITHUD:
                hit_label = self.text.get_label("BONK!", scale=3, color=(255, 50, 0))
    ...
    def start(self):
        ...
        while self.loop:
            # Checking we have received the data from UDP!!!
            data, addr = sock.recvfrom(1024) # buffer size is 1024 bytes
            current_time = datetime.now()
            current_time = current_time.strftime("%M:%S")
            print("Time: ", current_time, "received message: %d" % int(data))
            
            # Do all events
            clicked, hit, miss = self.loop_events(int(data)) 
